...
async function start() {
    try { 
        ...
        const result = await new Promise((done,reject) => {
            ...
            /* 
            Промисс здесь ушел в ожидание на следующие событийные итерации.
              Условно внутри системы это отработает примерно так process.nextTick(setTimeout)
              То есть таймер передастся диплексору событий в начале следущего цикла, и вернется от
              обработчика через примерно 1 секунду.
            */
            setTimeout(() => {
            ...
            // Не сработает, так как, выброс ошибки раньше.
            setTimeout(() => {
